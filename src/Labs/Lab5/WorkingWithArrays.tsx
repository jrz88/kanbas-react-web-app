...
export default function WorkingWithArrays() {
    ...
    return (
        <div id="wd-working-with-arrays">
            ...
            <h3>updates todos completed status</h3>

            <a href={`${API}/${todo.id}/completed/${todo.completed}`} className="btn btn-primary float-end">
                update todos completed status</a>
            <input value={todo.id} className="form-control w-25 float-start"
                onChange={(e) => setTodo({ ...todo, id: e.target.value })} />
            <input className="form-check-input" id="wd-completed-todos-true" type="checkbox"
                checked={todo.completed} onChange={(e) =>
                    setTodo({ ...todo, completed: true })} />
            <label className="form-check-label" htmlFor="wd-completed-todos-true">
                True
            </label>
            <input className="form-check-input " id="wd-completed-todos-false" type="checkbox"
                checked={!todo.completed} onChange={(e) =>
                    setTodo({ ...todo, completed: false })} />
            <label className="form-check-label" htmlFor="wd-completed-todos-false">
                False
            </label>

            <hr />

            <h3>updates todos description</h3>

            <a id="wd-update-todo-description" className="btn btn-primary float-end"
                href={`${API}/${todo.id}/description/${todo.description}`}>
                update todo description
            </a>
            <input id="wd-todo-id" value={todo.id} className="form-control w-25 float-start"
                onChange={(e) => setTodo({ ...todo, id: e.target.value })} />
            <input id="wd-todo-description" value={todo.description} className="form-control w-25"
